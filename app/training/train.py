...
from app.config.configs import model_path, model_name,param_grid
import logging
import joblib
import os
...
def train_model(x_train, y_train, x_test, y_test):
    ...
    try:
        ...
        if not os.path.exists(f'{model_path}/info.py'):
            os.makedirs(f'{model_path}/info.py')

        with open(f"{model_path}/info.py", "w") as file:
        ...
        logging.info(f"Training information saved to {model_path}/info.py")

        # Save the final best trained model and overwrite any previous version.
        joblib.dump(best_model, f"{model_path}/{model_name}")
        logging.info(f"Trained model saved to {model_path}/{model_name} \n")
